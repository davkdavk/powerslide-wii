...
#if defined(WII) || defined(__wii__)
    {
        WiiGX::Renderer& probeRenderer = WiiGX::Renderer::getInstance();
        probeRenderer.presentProbeColor(255, 255, 0, 255);
    }

    if(!fatInitDefault())
    {
        std::printf("[WII] fatInitDefault failed, sd:/powerslide assets unavailable\n");
    }
#endif
...
#if defined(WII) || defined(__wii__)
    // Gap-isolation step: disable all calls between BaseApp construction and go().
    // Previously in this gap:
    // 1) WiiGX::Renderer::getInstance()
    // 2) presentProbeColor(0,255,255,255)
    // 3) presentProbeColor(255,0,255,255)
#endif
