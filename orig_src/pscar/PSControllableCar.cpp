                                    ...
                                    bool isAI)
{
...
#if defined(WII) || defined(__wii__)
    if(!mModelNode || !mPhysicsVehicle)
    {
        WiiDebugLog("[MODEL] PSControllableCar::initModel Wii bypass tail (no model/physics)\n");
        return;
    }
#endif
...

    {
        ...
        if(freqLow.size() < expected || freqMid.size() < expected || freqHigh.size() < expected ||
           volLow.size() < expected || volMid.size() < expected || volHigh.size() < expected)
        {
            WiiDebugLog("[MODEL] PSControllableCar::initModel invalid spline array sizes, using defaults\n");
            freqLow.assign(expected, "1.0");
            freqMid.assign(expected, "1.0");
            freqHigh.assign(expected, "1.0");
            volLow.assign(expected, "0.2");
            volMid.assign(expected, "0.2");
            volHigh.assign(expected, "0.2");
        }
#endif
        ...
        {
            float val;
            if(!Conversions::DMFromString(freqLow[q], val)) val = defaultLowPitch;
            mPitchValueLow.addPoint(static_cast<float>(q) * 1000.0f, val * hscale0);

            if(!Conversions::DMFromString(freqMid[q], val)) val = defaultMidPitch;
            mPitchValueMid.addPoint(static_cast<float>(q) * 1000.0f, val * hscale1);

            if(!Conversions::DMFromString(freqHigh[q], val)) val = defaultHighPitch;
            mPitchValueHigh.addPoint(static_cast<float>(q) * 1000.0f, val * hscale2);

            if(!Conversions::DMFromString(volLow[q], val)) val = 0.2f;
            mGainValueLow.addPoint(static_cast<float>(q) * 1000.0f, val);

            if(!Conversions::DMFromString(volMid[q], val)) val = 0.2f;
            mGainValueMid.addPoint(static_cast<float>(q) * 1000.0f, val);

            if(!Conversions::DMFromString(volHigh[q], val)) val = 0.2f;
