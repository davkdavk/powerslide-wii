                            ...
                            bool isAdvancedLighting)
{
    ...
    (void)modelsPool;
    (void)characterName;
    (void)initialVehicleSetup;
    (void)isAdvancedLighting;
    ...
    {
        mWheelNodes[q] = NULL;
        mWheelEntitys[q] = NULL;
    }
    mModelNode = NULL;
    WiiDebugLog("[MODEL] PSBaseGraphicsVehicle::initGraphicsModel Wii bypass\n");
    return;
#endif
...
std::string PSBaseGraphicsVehicle::loadTexture(const GameState& gameState, const std::string& textureName)
{
    ...
    //load car texture
    {
        ...
        TEXLoader().load(gameState.getPFLoaderData(), "data/cars/" + carPath + "/textures/default/" + mCharacterName, carSkinName, textureName, gameState.getX2LUTs(), gameState.getX4LUTs(), TEMP_RESOURCE_GROUP_NAME, gameState.getGamma(), gameState.getDoUpscale());
    }
