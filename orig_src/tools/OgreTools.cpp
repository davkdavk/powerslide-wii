                                ...
                                std::vector<std::vector<Ogre::Vector3> >& suspensionPointOriginalPos)
{
    ...
    (void)entity;
    (void)suspensionData;
    suspensionIndices.assign(4, std::vector<size_t>());
    suspensionPointOriginalPos.assign(4, std::vector<Ogre::Vector3>());
    WiiDebugLog("[MODEL] AdjustBufferToUseShadow: Wii bypass\n");
    return;
    ...
    if(!entity)
    {
        suspensionIndices.assign(4, std::vector<size_t>());
        suspensionPointOriginalPos.assign(4, std::vector<Ogre::Vector3>());
        WiiDebugLog("[MODEL] AdjustBufferToUseShadow: null entity\n");
        return;
    }

    Ogre::Mesh* shadowMesh = entity->getMesh().get();
    if(!shadowMesh || !shadowMesh->sharedVertexData)
    {
        suspensionIndices.assign(4, std::vector<size_t>());
        suspensionPointOriginalPos.assign(4, std::vector<Ogre::Vector3>());
        WiiDebugLog("[MODEL] AdjustBufferToUseShadow: missing mesh/shared data\n");
        return;
    }
#endif
...
Ogre::Real GetSignedAngle(const Ogre::Vector3& a, const Ogre::Vector3& b)
{
    ...
    Ogre::Radian angleRad;
    rot.ToAngleAxis(angleRad, axis);
    Ogre::Real res = angleRad.valueRadians * (180.0f / 3.1415926535f);
...
Ogre::DataStreamPtr getReadableFile(const std::string& dataDir, const std::string& fileName)
{
    ...
    else//android
    {
        ...
        if(!PFLoader::mPackedFileSubdir.empty())
            fullPath += PFLoader::mPackedFileSubdir + "/";
        fullPath += fileName;
        Ogre::FileStreamDataStream* streamtmp = 0;
        streamtmp = OGRE_NEW Ogre::FileStreamDataStream(fullPath.c_str(), false);

        if(streamtmp->isReadable())
        {
...
Ogre::DataStreamPtr getWritibleFile(const std::string& dataDir, const std::string& fileName)
{
    ...
    else //android
    {
        ...
        fullPath += fileName;
        Ogre::FileStreamDataStream* streamtmp = 0;
        streamtmp = OGRE_NEW Ogre::FileStreamDataStream(fullPath.c_str(), true);

        if(streamtmp->isWriteable())
        {
            stream = Ogre::DataStreamPtr(streamtmp);
        }
        else
