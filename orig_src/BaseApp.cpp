...
namespace
{
    ...
    {
        // WPAD is disabled for Dolphin bring-up stability.
    }
#endif
...
{
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(255, 128, 0, 255);
#endif
...
#if defined(WII) || defined(__wii__)
        WiiGX::Renderer::getInstance().presentProbeColor(0, 128, 255, 255);
#endif
...
void BaseApp::go(bool isSafeRun)
{
    ...
    const bool setupOk = setup();

    if (!setupOk)
    {
        std::printf("[WII] BaseApp::setup failed, entering blank-screen fallback loop\n");
        ...
        if (!mWindow)
        {
            mWindow = mRoot->initialise(true, "Powerslide Remake (Fallback)");
        }
...
{
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 255, 0, 255);
#endif

    Ogre::gSetupCalled = true;
    mRoot = std::make_shared<Ogre::Root>();
    mOverlaySystem = std::make_shared<CustomOverlaySystem>();

#if defined(WII) || defined(__wii__)
    mGameState.setDataDir("sd:/apps/powerslide");
#endif
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(255, 0, 0, 255);
#endif

    if (mLuaError )
    {
#if defined(WII) || defined(__wii__)
        Ogre::LogManager::getSingleton().logMessage(Ogre::LML_NORMAL, "[BaseApp::setup]: Lua error on Wii, continuing for render bring-up");
        mLuaError = 0;
#else
        return false;
#endif
    ...
    {
#if defined(WII) || defined(__wii__)
        Ogre::LogManager::getSingleton().logMessage(Ogre::LML_NORMAL, "[BaseApp::setup]: Packed data missing, continuing for render bring-up");
#else
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 0, 255, 255);
#endif
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(255, 0, 128, 255);
#endif

    Ogre::ResourceGroupManager::getSingleton().initialiseResourceGroup("General");

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 255, 64, 255);
#endif
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 255, 192, 255);
#endif
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(255, 192, 0, 255);
#endif
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(255, 96, 0, 255);
#endif

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(255, 64, 0, 255);
#endif

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(255, 32, 0, 255);
#endif

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(255, 16, 0, 255);
#endif
...
#if defined (__ANDROID__)
#else
    if(mInputHandler && mInputHandler->getInputContext().mMouse)
    {
...
bool BaseApp::setShutdown(bool isOnEsc)
{
    if(mGameModeSwitcher->isLoadPassed())
    {
        ...
        {
            if(
                mGameModeSwitcher->getSubmenuState() != State_Options_Trophies && 
                mGameModeSwitcher->getSubmenuState() != State_Options_Trophies_Video && 
                mGameModeSwitcher->getSubmenuState() != State_Options_Credits_Video &&
                mGameModeSwitcher->getSubmenuState() != State_Options_About_Video &&
                mGameModeSwitcher->getSubmenuState() != State_Options_Name
                )
            {
...
void BaseApp::touchReleased(const OIS::MultiTouchEvent& arg)
{
    ...
    if(mGameModeSwitcher->isLoadPassed())
    {
        //if(mGameModeSwitcher->getMode() == ModeMenu || mGameModeSwitcher->getMode() == ModeMenuMulti)
        {
            ...
            mGameModeSwitcher->mouseReleased(Ogre::Vector2(arg.state.X.abs, arg.state.Y.abs), OIS::MB_Left);
        }
...
{
 #if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(128, 0, 255, 255);
 #endif
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(96, 0, 255, 255);
#endif
