...
void ExclusionLoader::load(GameState& gameState, Ogre::SceneManager* sceneMgr, bool isDebugExclusion) const
{
    ...
    {
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[EXCL] file open ok '%s'\n", excludeFile.c_str());
#endif
...
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[EXCL] load end exclusions=%u\n", static_cast<unsigned int>(gameState.getExclusions().size()));
#endif
    }
#if defined(WII) || defined(__wii__)
    else
    {
        WiiDebugLog("[EXCL] missing '%s'\n", excludeFile.c_str());
    }
#endif
}
