...
#if defined(WII) || defined(__wii__)
    Ogre::uint32 swap32(Ogre::uint32 v)
    {
...
#if defined(WII) || defined(__wii__)
        head.width = swap32(head.width);
        head.height = swap32(head.height);
        head.depth = swap32(head.depth);

        if(head.width == 0 || head.height == 0 || head.width > 4096 || head.height > 4096)
        {
            WiiDebugLog("[TEX] invalid header name='%s' w=%u h=%u d=%u\n",
                texturename.c_str(),
                static_cast<unsigned int>(head.width),
                static_cast<unsigned int>(head.height),
                static_cast<unsigned int>(head.depth));
            return res;
        }
#endif
                                            ...
                                            size_t newSize) const
{
    ...
    if(fileToLoad.get() && fileToLoad->isReadable())
    {
        ...
        if(head.width == 0 || head.height == 0 || head.width > 4096 || head.height > 4096)
        {
            WiiDebugLog("[TEX] invalid chroma header name='%s' w=%u h=%u d=%u\n",
                texturename.c_str(),
                static_cast<unsigned int>(head.width),
                static_cast<unsigned int>(head.height),
                static_cast<unsigned int>(head.depth));
