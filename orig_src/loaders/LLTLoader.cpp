...
{
#if defined(WII) || defined(__wii__)
    static Ogre::uint32 readLE32(const Ogre::DataStreamPtr& stream)
    {
    ...
    static float readLEFloat(const Ogre::DataStreamPtr& stream)
    {
        const Ogre::uint32 bits = readLE32(stream);
        union
        {
            Ogre::uint32 u;
            float f;
...
void LLTLoader::load(GameState& gameState, Ogre::SceneManager* sceneMgr, bool isDebugLLT) const
{
...
#if defined(WII) || defined(__wii__)
    WiiDebugLog("[LLT] track=%s baseDir=%s\n", gameState.getTrackName().c_str(), baseDir.c_str());
    WiiDebugLog("[LLT] PF inited data=%d store=%d\n",
        gameState.getPFLoaderData().isInited() ? 1 : 0,
        gameState.getPFLoaderStore().isInited() ? 1 : 0);

    {
        ...
        const char* probes[] = {
            ...
            "sd:/data.pf",
            "sd:/powerslide/data.pf",
            "sd:/powerslide/store.pf"
        };
        for(size_t p = 0; p < sizeof(probes) / sizeof(probes[0]); ++p)
        {
            const int ok = ::stat(probes[p], &st);
            WiiDebugLog("[LLT] stat %s => %d\n", probes[p], ok == 0 ? 1 : 0);
        }
    }

    if(baseDir.empty())
    {
        const std::string strDir = "data/tracks";
        const std::string strFile = "powerslide.str";
        Ogre::DataStreamPtr strData = gameState.getPFLoaderStore().getFile(strDir, strFile);
        if(strData.get() && strData->isReadable())
        {
            const size_t sz = strData->size();
            std::string buf;
            buf.resize(sz);
            const size_t rd = strData->read(&buf[0], sz);
            WiiDebugLog("[LLT] powerslide.str bytes=%u read=%u\n", static_cast<unsigned int>(sz), static_cast<unsigned int>(rd));

            const std::string section = "[" + gameState.getTrackName() + " parameters]";
            const size_t secPos = buf.find(section);
            WiiDebugLog("[LLT] section '%s' found=%d\n", section.c_str(), secPos == std::string::npos ? 0 : 1);

            if(secPos != std::string::npos)
            {
                const size_t nextSec = buf.find("\n[", secPos + 1);
                const size_t endPos = nextSec == std::string::npos ? buf.size() : nextSec;
                const std::string chunk = buf.substr(secPos, endPos - secPos);

                const char* key = "base directory =";
                const size_t keyPos = chunk.find(key);
                WiiDebugLog("[LLT] key '%s' found=%d\n", key, keyPos == std::string::npos ? 0 : 1);
                if(keyPos != std::string::npos)
                {
                    size_t valStart = keyPos + std::strlen(key);
                    while(valStart < chunk.size() && (chunk[valStart] == ' ' || chunk[valStart] == '\t'))
                        ++valStart;
                    size_t valEnd = valStart;
                    while(valEnd < chunk.size() && chunk[valEnd] != '\r' && chunk[valEnd] != '\n')
                        ++valEnd;
                    const std::string rawVal = chunk.substr(valStart, valEnd - valStart);
                    WiiDebugLog("[LLT] raw base directory value='%s'\n", rawVal.c_str());
                }
            }
        ...
        {
            WiiDebugLog("[LLT] unable to open powerslide.str via PFLoaderStore\n");
        }
    }
#endif
    ...
    Ogre::DataStreamPtr fileToLoad;
    {
        ...
        candidateDirs.push_back("data/tracks/" + baseDir + "/graphst");
        candidateDirs.push_back("data/tracks/" + baseDir + "/GRAPHST");
        candidateDirs.push_back("data/tracks/" + baseDir + "/graphs");
        candidateDirs.push_back("data/tracks/" + baseDir);
        candidateDirs.push_back("data/tracks");
        candidateDirs.push_back("graphst");
        candidateDirs.push_back("");

        std::vector<std::string> candidateFiles;
        candidateFiles.push_back("laptrack.lpf");
        candidateFiles.push_back("LAPTRACK.LPF");
        candidateFiles.push_back("laptrack.LPF");

        for(size_t d = 0; d < candidateDirs.size() && (!fileToLoad.get() || !fileToLoad->isReadable()); ++d)
        {
            for(size_t f = 0; f < candidateFiles.size() && (!fileToLoad.get() || !fileToLoad->isReadable()); ++f)
            {
#if defined(WII) || defined(__wii__)
                WiiDebugLog("[LLT] try dir='%s' file='%s'\n", candidateDirs[d].c_str(), candidateFiles[f].c_str());
#endif
                Ogre::DataStreamPtr candidate = gameState.getPFLoaderData().getFile(candidateDirs[d], candidateFiles[f]);
                if(candidate.get() && candidate->isReadable())
                {
                    fileToLoad = candidate;
#if defined(WII) || defined(__wii__)
                    WiiDebugLog("[LLT] open ok dir='%s' file='%s' size=%u\n", candidateDirs[d].c_str(), candidateFiles[f].c_str(), static_cast<unsigned int>(fileToLoad->size()));
#endif
                }
            }
        }
    ...
    {
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[LLT] file ptr ok, readable=%d size=%u\n",
            fileToLoad->isReadable() ? 1 : 0,
    ...
    if(sceneMgr)
    {
        WiiDebugLog("[LLT] sceneMgr ok\n");
    }
    else
    {
        WiiDebugLog("[LLT] sceneMgr null\n");
    }
#endif
    ...
    if(fileToLoad.get() && fileToLoad->isReadable())
    {
        ...
        Ogre::uint32 flag = 0;
#if defined(WII) || defined(__wii__)
        flag = readLE32(fileToLoad);
        const size_t readFlag = 4;
#else
        const size_t readFlag = fileToLoad->read(&flag, 4);
#endif
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[LLT] read flag bytes=%u flag=%u\n", static_cast<unsigned int>(readFlag), static_cast<unsigned int>(flag));
#endif
        if(flag <= 1)
        {
            char LLTINFO[8];
            const size_t readHeader = fileToLoad->read(LLTINFO, 8);
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[LLT] read LLTINFO bytes=%u raw='%.8s'\n", static_cast<unsigned int>(readHeader), LLTINFO);
#endif
            if(std::string(LLTINFO) == "LLTINFO")
            {
                Ogre::uint32 someCount = 0;    // checkpoints count
#if defined(WII) || defined(__wii__)
                someCount = readLE32(fileToLoad);
                const size_t readSomeCount = 4;
#else
                const size_t readSomeCount = fileToLoad->read(&someCount, 4);
#endif

#if defined(WII) || defined(__wii__)
                WiiDebugLog("[LLT] someCount bytes=%u value=%u\n", static_cast<unsigned int>(readSomeCount), static_cast<unsigned int>(someCount));
#endif
                ...
                {
#if defined(WII) || defined(__wii__)
                    something = readLE32(fileToLoad);
                    const size_t readSomething = 4;
#else
                    const size_t readSomething = fileToLoad->read(&something, 4);
#endif
#if defined(WII) || defined(__wii__)
                    WiiDebugLog("[LLT] something bytes=%u value=%u\n", static_cast<unsigned int>(readSomething), static_cast<unsigned int>(something));
#endif
                }

                Ogre::uint32 someCount3 = 0;
                Ogre::uint32 someCount2 = 0;
#if defined(WII) || defined(__wii__)
                someCount3 = readLE32(fileToLoad);
                someCount2 = readLE32(fileToLoad);
                const size_t readSomeCount3 = 4;
                const size_t readSomeCount2 = 4;
#else
                const size_t readSomeCount3 = fileToLoad->read(&someCount3, 4);
                const size_t readSomeCount2 = fileToLoad->read(&someCount2, 4); // track points count
#endif

#if defined(WII) || defined(__wii__)
                WiiDebugLog("[LLT] someCount3 bytes=%u value=%u\n", static_cast<unsigned int>(readSomeCount3), static_cast<unsigned int>(someCount3));
                WiiDebugLog("[LLT] someCount2 bytes=%u value=%u\n", static_cast<unsigned int>(readSomeCount2), static_cast<unsigned int>(someCount2));
#endif

                if(someCount2 == 0)
                {
#if defined(WII) || defined(__wii__)
                    WiiDebugLog("[LLT] gate: someCount2 is zero, no vertices to read\n");
#endif
                }
                else
                {
#if defined(WII) || defined(__wii__)
                    WiiDebugLog("[LLT] entering vertex loop count=%u\n", static_cast<unsigned int>(someCount2));
#endif
                }
                ...
                for(size_t q = 0; q < someCount2; ++q)
                {
...
#if defined(WII) || defined(__wii__)
                    const Ogre::uint32 some_flag = readLE32(fileToLoad);
                    const float x = readLEFloat(fileToLoad);
                    const float y = readLEFloat(fileToLoad);
                    const float z = readLEFloat(fileToLoad);
                    const float weight = readLEFloat(fileToLoad);
                    const Ogre::uint32 nextIndex = readLE32(fileToLoad);
                    (void)readLE32(fileToLoad);
                    const float n1 = readLEFloat(fileToLoad);
                    const float n2 = readLEFloat(fileToLoad);
                    const float n3 = readLEFloat(fileToLoad);
                    ...
                    tp.index = nextIndex;
                    tp.weight = weight;
                    tp.isCheckpoint = some_flag == 1 ? true : false;
                    tp.dir.x = n1;
                    tp.dir.y = n2;
                    tp.dir.z = -n3;
#else
                    ...
                    if(q == 0)
                    {
                        WiiDebugLog("[LLT] first vertex %.3f %.3f %.3f idx=%u\n", tp.vert.x, tp.vert.y, tp.vert.z, static_cast<unsigned int>(tp.index));
                    }
                    else if(q == 1)
                    {
                        WiiDebugLog("[LLT] second vertex %.3f %.3f %.3f idx=%u\n", tp.vert.x, tp.vert.y, tp.vert.z, static_cast<unsigned int>(tp.index));
                    }
#endif
            ...
            {
#if defined(WII) || defined(__wii__)
                WiiDebugLog("[LLT] gate: LLTINFO header mismatch\n");
#endif
            }
        ...
        {
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[LLT] gate: flag validation failed (flag=%u)\n", static_cast<unsigned int>(flag));
#endif
        }
        ...
        //if(endendFound)
        {
...
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[LLT] parsed track points=%u\n", static_cast<unsigned int>(track.size()));
#endif
...
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[LLT] vectors built positions=%u checkpoints=%u\n",
                static_cast<unsigned int>(positions.size()),
                static_cast<unsigned int>(isCheckpoints.size()));
#endif
            ...
            {
#if defined(WII) || defined(__wii__)
                WiiDebugLog("[LLT] debug ManualObjects disabled on Wii\n");
#else
#if defined(WII) || defined(__wii__)
                WiiDebugLog("[LLT] debug draw path enabled\n");
#endif
                if(!positions.empty())
                {
#if defined(WII) || defined(__wii__)
                    WiiDebugLog("[LLT] creating debug line object\n");
                    WiiDebugLog("[LLT] debug line points=%u\n", static_cast<unsigned int>(positions.size()));
#endif
                    ...
                    WiiDebugLog("[LLT] forcing LLT debug object visible on Wii\n");
#endif
                    
                    lltNode->attachObject(gameState.getLLTObject());
#if defined(WII) || defined(__wii__)
                    WiiDebugLog("[LLT] debug line object attached\n");
#endif
                }
                ...
                {
#if defined(WII) || defined(__wii__)
                    WiiDebugLog("[LLT] creating checkpoint debug markers\n");
#endif
                ...
                {
#if defined(WII) || defined(__wii__)
                    WiiDebugLog("[LLT] creating direction debug markers\n");
#endif
...
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[LLT] lap data distributed\n");
#endif
        }

        fileToLoad->close();
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[LLT] load() end success\n");
#endif
    }
    else {
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[LLT] gate: file unreadable or not found\n");
#endif
        assert(false && "No LLT file");
    }
}
