...
Ogre::TexturePtr TextureLoader::load(const PFLoader& pfLoader, const std::string& subfolder, const std::string& filename, const std::string& texturename, const Ogre::String& group, Ogre::Real gamma) const
{
    ...
    {
        Ogre::LogManager::getSingleton().logMessage(Ogre::LML_NORMAL, "[TextureLoader::load]: Missing texture [" + filename + "], using generated fallback");
        res = generate(texturename, 64, 64, Ogre::ColourValue(1.0f, 1.0f, 1.0f, 1.0f), group, Ogre::PF_A8R8G8B8);
    }
                                            ...
                                            bool blackForTransparent) const
{
    ...
    {
        Ogre::LogManager::getSingleton().logMessage(Ogre::LML_NORMAL, "[TextureLoader::loadChroma]: Missing texture [" + filename + "], using generated fallback");
        res = generate(texturename, static_cast<Ogre::uint>(newSize), static_cast<Ogre::uint>(newSize), Ogre::ColourValue(1.0f, 1.0f, 1.0f, 1.0f), group, Ogre::PF_A8R8G8B8);
    }
