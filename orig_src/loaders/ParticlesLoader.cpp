...
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[PART] load end particles=%u\n", static_cast<unsigned int>(gameState.getParticles().size()));
#endif
    }
    else
    {
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[PART] missing partgraph.txt\n");
#endif
        assert(false && "No particle file");
    }
}
