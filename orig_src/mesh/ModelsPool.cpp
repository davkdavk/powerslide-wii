...
Ogre::Entity* ModelsPool::createFallbackArrowEntity()
{
    if(!mSceneMgr)
        return NULL;

    Ogre::String fallbackName = nameGenMeshes.generate();
    return mSceneMgr->createEntity(fallbackName, Ogre::SceneManager::PT_SPHERE);
}
...
void ModelsPool::initModels(Ogre::SceneManager* sceneMgr, const GameState& gameState)
{
...
#if defined(WII) || defined(__wii__)
    WiiDebugLog("[MODEL] initModels: using Wii fallback arrow\n");
    mArrowModel = createFallbackArrowEntity();
#else
    ...
    if(!mArrowModel)
    {
        mArrowModel = createFallbackArrowEntity();
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[MODEL] arrow fallback in initModels\n");
#endif
    }
}
...
void ModelsPool::loadArrow(const PFLoader& pfloader)
{
    ...
    if(fileToLoad.get() && fileToLoad->isReadable())
    {
        ...
        if(!mArrowModel)
        {
            mArrowModel = createFallbackArrowEntity();
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[MODEL] arrow fallback after parse\n");
#endif
        }
    }
    else
    {
        mArrowModel = createFallbackArrowEntity();
#if defined(WII) || defined(__wii__)
        WiiDebugLog("[MODEL] missing arrow.de2, using fallback\n");
#endif
    }
}
...
void ModelsPool::loadVehicle(const PFLoader& pfloader, vehicleModel& vehicle, const std::string& path, const std::string& fileName)
{
    ...
    if(fileToLoad.get() && fileToLoad->isReadable())
    {
        ...
        {
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[MODEL] failed parse vehicle de2 '%s/%s'\n", path.c_str(), fileName.c_str());
#endif
        }
        fileToLoad->close();
    }
    ...
    {
        Ogre::LogManager::getSingleton().logMessage(Ogre::LML_CRITICAL, "[ModelsPool::loadVehicle]: missing DE2 " + Ogre::String((path + "/" + fileName).c_str()));
    }
}
...
void ModelsPool::getCopyOfVehicle(const GameState& gameState, const std::string& vehicleName, Ogre::Entity* vehicleModel[5])
{
    ...
    {
        Ogre::String fallbackName = nameGenMeshes.generate();
        vehicleModel[q] = mSceneMgr->createEntity(fallbackName, Ogre::SceneManager::PT_SPHERE);
    }
    WiiDebugLog("[MODEL] getCopyOfVehicle: Wii fallback spheres\n");
    return;
#endif
    ...
    for(size_t q = 0; q < 5; ++q)
    {
        if(!model.mVehicleModel[q])
        {
            Ogre::String fallbackName = nameGenMeshes.generate();
            vehicleModel[q] = mSceneMgr->createEntity(fallbackName, Ogre::SceneManager::PT_SPHERE);
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[MODEL] vehicle '%s' part%u missing, fallback sphere\n", vehicleName.c_str(), static_cast<unsigned int>(q));
#endif
            continue;
        }
        ...
        if(!msh.get())
        {
            ...
            vehicleModel[q] = mSceneMgr->createEntity(fallbackName, Ogre::SceneManager::PT_SPHERE);
#if defined(WII) || defined(__wii__)
            WiiDebugLog("[MODEL] vehicle '%s' part%u no mesh, fallback sphere\n", vehicleName.c_str(), static_cast<unsigned int>(q));
#endif
            continue;
        }
        ...
        if(!vehicleModel[q])
        {
            vehicleModel[q] = mSceneMgr->createEntity(nameGenMeshes.generate(), Ogre::SceneManager::PT_SPHERE);
        }
    }
}
