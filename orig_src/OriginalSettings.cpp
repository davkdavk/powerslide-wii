...
std::string STRPowerslide::getBaseDir(const std::string& trackName) const
{
    ...
    if(ret.empty())
    {
        static std::map<std::string, std::string> fallbackBaseDirs;
        if(fallbackBaseDirs.empty())
        {
            fallbackBaseDirs["desert track"] = "desert";
            fallbackBaseDirs["speedway track"] = "speedway";
            fallbackBaseDirs["speedway night track"] = "speedway";
            fallbackBaseDirs["mountain forest track"] = "mf";
            fallbackBaseDirs["mineshaft"] = "mineshaft";
            fallbackBaseDirs["alpine track"] = "alpine";
            fallbackBaseDirs["city track"] = "citytrack";
            fallbackBaseDirs["nutopia track"] = "nutopia";
            fallbackBaseDirs["stunt track"] = "stunt";
            fallbackBaseDirs["luge track"] = "luge";
            fallbackBaseDirs["Foxnhound1 track"] = "foxnhound1";
            fallbackBaseDirs["Foxnhound2 track"] = "foxnhound2";
            fallbackBaseDirs["dam track"] = "dam";
        }

        std::map<std::string, std::string>::const_iterator i = fallbackBaseDirs.find(trackName);
        if(i != fallbackBaseDirs.end())
            ret = (*i).second;
    }
