...
std::vector<std::string> RacingGridGeneration::generate(GameState& gameState, const std::vector<std::string>& playersCharacters, std::vector<size_t>& resAISlot) const
{
    ...
    const size_t aiCountInRace = gameState.getAICountInRace();
    const std::string fallbackCharacter = "frantic";

    auto resolveAIIndex = [&](size_t raceOrderIndex) -> size_t
    {
        if(aiLinearIndexes.empty() || aiIndexes.empty())
        {
            return 0;
        }

        if(raceOrderIndex >= aiLinearIndexes.size())
        {
            raceOrderIndex = aiLinearIndexes.size() - 1;
        }

        size_t aiIndex = aiLinearIndexes[raceOrderIndex];
        if(aiIndex >= aiIndexes.size())
        {
            aiIndex = aiIndexes.size() - 1;
        }

        return aiIndexes[aiIndex];
    };

    auto getCharacterByAiIndex = [&](size_t aiIndex) -> std::string
    {
        ...
        if(aiIndex >= availableCharacters.size())
        {
            aiIndex %= availableCharacters.size();
        }

        return availableCharacters[aiIndex];
    };

    for(size_t q = 0; q < gameState.getAICountInRace(); ++q)
    {
        const size_t raceOrderIndex = aiCountInRace - q - 1;
        const size_t aiIndex = resolveAIIndex(raceOrderIndex);
        res.push_back(getCharacterByAiIndex(aiIndex));
    }
    ...
    for(size_t q = 0; q < gameState.getAICountInRace(); ++q)
    {
        ...
        if(gameState.getAIStrength() == Insane || gameState.getAIStrength() == UltraInsane)
            resAISlot.push_back(getSlotInsaneIndex(aiIndex));
        else
            resAISlot.push_back(getSlotIndex(aiIndex));
    }
    ...
    for(size_t q = 0; q < playersCharacters.size(); ++q)
    {
        ...
        if(found != res.end())
        {
            ...
            while(newCharIndex < GameState::mRaceGridCarsMax)
            {
                ...
                if(!aiIndexes.empty())
                {
                    aiIndex = aiIndexes[newCharIndex % aiIndexes.size()];
                }

                std::string newCharName = getCharacterByAiIndex(aiIndex);
                if(newCharName != playerCharacter)
                {
                    ...
                    if(foundMore == res.end())
                    {
                        ...
                        if(gameState.getAIStrength() == Insane)
                            resAISlot[foundIndex] = getSlotInsaneIndex(aiIndex);
                        else
                            resAISlot[foundIndex] = getSlotIndex(aiIndex);
