    ...
    mIsRecreate(false)
{
    ...
    WiiGX::Renderer::getInstance().presentProbeColor(0, 224, 255, 255);
#endif

    mContext.setGameModeSwitcher(this);

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 192, 255, 255);
#endif

    mMenuMode = std::make_shared<MenuMode>(mContext, ModeMenu, State_Options_Name);

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 160, 255, 255);
#endif

#if defined(WII) || defined(__wii__)
    // Wii bring-up: skip full menu bootstrap and go straight to race init path.
#else
    mUIBackground.show();
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 96, 255, 255);
#endif
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 64, 255, 255);
#endif
...
#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 32, 255, 255);
#endif

    mMenuMode->initData(this);

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(0, 16, 255, 255);
#endif

    mUIBackground.hide();

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(32, 0, 255, 255);
#endif

    mMenuMode->initCamera();

#if defined(WII) || defined(__wii__)
    WiiGX::Renderer::getInstance().presentProbeColor(64, 0, 255, 255);
#endif
#endif
    ...
    // Wii bring-up: force immediate transition state so frameEnded enters race init branch.
    mGameMode = ModeMenu;
    mGameModeNext = ModeRaceSingle;
    mIsSwitchMode = true;

    WiiGX::Renderer::getInstance().presentProbeColor(0, 240, 255, 255);
    frameEnded();

    // Prevent re-entry: after initial switch, lock to race mode.
...
void GameModeSwitcher::frameEnded()
{
    ...
    if(modeRace)
    {
        // Native Wii terrain bring-up: keep race mode stable even when
        // vehicle/physics init is intentionally stubbed.
        mGameModeNext = mGameMode;
        mIsSwitchMode = false;
    }
#endif
...
#if defined(WII) || defined(__wii__)
    // Wii bring-up: prevent race-over switch when player physics is null
    // (race "finishes" immediately without physics, causing infinite loop)
    raceOverAndReadyToQuit = false;
#endif
...
void GameModeSwitcher::loadState(float percent, const std::string& info)
{
    ...
    {
        WiiGX::Renderer::getInstance().presentProbeColor(16, 16, 136, 255);
        return;
    }
#endif
    ...
    {
        if(mIsInitialLoadPassed && mUIUnloader)
